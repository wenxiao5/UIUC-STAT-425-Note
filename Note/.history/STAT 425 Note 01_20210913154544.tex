...
Then we can compute the \textbf{Maximum likelihood estimators} $(\hat{\beta}_0,\hat{\beta}_1,\hat{\sigma}^2)$: obviously, maximizing $L(\beta_0,\beta_1,\sigma^2)$ is as same as minimizing $\sum_{i=1}^n(y_i-(\beta_0+\beta_1x_i))^2$, then the \textbf{Maximum likelihood estimators} is exactly the \textbf{LS estimators}:
$$\begin{aligned}
