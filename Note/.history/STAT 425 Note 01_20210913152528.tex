...
Then we can compute the \textbf{Maximum likelihood estimators}: obviously, maximizing $L(\beta_0,\beta_1,\sigma^2)$ is as same as minimizing $\sum_{i=1}^n(y_i-(\beta_0+\beta_1x_i))^2$, then the \textbf{Maximum likelihood estimators} is exactly the \textbf{LS estimators}:
$$\frac{\partial L}{\partial \beta_1}=\frac{1}{2\sigma^2}\sum_{i=1}^nx_i(y_i-\beta_0-\beta_1x_i)$$
$$\frac{\partial L}{\partial \beta_0}=\frac{1}{2\sigma^2}\sum_{i=1}^n(y_i-\beta_0-\beta_1x_i) \Rightarrow \hat{\beta}_0=\bar{y}-\hat{\beta}_1 \bar{x}$$
