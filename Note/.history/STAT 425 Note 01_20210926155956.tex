...
\begin{equation}
    \begin{aligned}
        Y &\sim X_{1}+\ldots+X_{i-1}+X_{i+1}+\ldots \\
X_{i} &\sim X_{1}+\ldots+X_{i-1}+X_{i+1}+\ldots
    \end{aligned}
    \begin{aligned}
        Y &\sim X_{1}+\ldots+X_{i-1}+X_{i+1}+\ldots \\
X_{i} &\sim X_{1}+\ldots+X_{i-1}+X_{i+1}+\ldots
...
$$
\begin{aligned}
\mathbf{r}_{y} &=\text { residuals from } \\
r_{k}^{x} &=\text { residuals from }(1)
\end{aligned}
$$
Plot $\mathrm{r}_{y}$ vs. $r_{k}^{X}$ : For a valid model, the added-variable plot should produce points randomly scattered around a line through the origin with slope $\hat{\beta}_{k}$. This is also a useful plot to detect high influential data points.
