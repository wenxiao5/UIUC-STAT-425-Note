...
\subsection{Testing a Single Predictor (Coeﬃcient)}
\subsubsection{Test $H_0: \beta_j=0$: $t-$test}
...
$$
\frac{\hat{\beta}_{1}-c}{\operatorname{se}\left(\hat{\beta}_{1}\right)}=\frac{\hat{\beta}_{1}-c}{\hat{\sigma} / \sqrt{S_{X X}}} \sim T_{n-2}, \hat{\sigma}^{2}=\frac{R S S}{n-2}
$$
$-$ In $M L R$ with $p$ predictors (including the intercept): Test for $\beta_{j}=c$,
$$
\frac{\hat{\beta}_{j}-c}{\operatorname{se}\left(\hat{\beta}_{j}\right)}=\frac{\hat{\beta}_{j}-c}{\hat{\sigma} \sqrt{\left[\left(\mathbf{X}^{T} \mathbf{X}\right)^{-1}\right]_{j j}}} \sim T_{n-p}, \hat{\sigma}^{2}=\frac{R S S}{n-p}
$$
