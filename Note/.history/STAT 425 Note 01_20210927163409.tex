...
$$\text{BP}=nR^2$$
where $R^2$is the \textit{coeﬃcient of Determination} between the squared residuals $r_i^2$ of a LS regression between Y and variables X 1 , X 2 , . . . , Xp  (including the intercept), and the covariates (or a sub-set) X 1 , X 2 , . . . , X p .
$$
...
\subsection{What happens if approximate collinearity: based on $\left(\frac{1}{1-R_{k}^{2}}\right)$ ($k-$th variance inﬂation factor (VIF))}
