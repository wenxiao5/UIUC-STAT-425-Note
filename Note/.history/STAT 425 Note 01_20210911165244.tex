...
\subsection{Residuals $\mathbf{r}=(\mathbf{I}-\mathbf{H}) y$, (Sample) error variance $\hat{\sigma}^2=\frac{R S S}{n-p}$}
$$
...
\subsubsection{$\mathbf{X}^{T} \boldsymbol{r}=0,\ \hat{y}^{T} \boldsymbol{r}=0$}
