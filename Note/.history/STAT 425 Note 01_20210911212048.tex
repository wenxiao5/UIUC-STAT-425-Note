...
$$
y_{i}=\mathbf{x}_{i}^{T} \beta+\varepsilon_{i} i=1, \ldots, n, \text { with } \varepsilon_{i} \sim N\left(0, \sigma^{2}\right)
$$
This implies that $\mathbf{y} \sim \mathbf{N}_{n}\left(\mathbf{X} \beta, \sigma^{2} \|\right)$
...
$$
The value of $\beta$ that maximizes the Likelihood function is the Maximum Likelihood Estimator (MLE) of $\beta$
This estimator is equal to the LS estimate of $\beta$.
