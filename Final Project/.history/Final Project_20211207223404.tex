...
In order to check the assumptions, we run the studentized Breusch-Pagan test (Figure 4), Kolmogorov-Smirnov test (Figure 5) and plot the diagnostic plots (Figure 6) which includes 1. Residuals vs. Fitted plot; 2. Normal Q-Q plot.\\
1. Constancy of Variance (Homoscedasticity): By the Residuals vs. Fitted plot and studentized Breusch-Pagan test result ($p-value=0.117>0.05$), we can conclude homoscedasticity assumption holds in the additive model.\\
2. Normality: By the Q-Q plot (obviously not a line) and Kolmogorov-Smirnov test result ($p-value=1.156e-07$), we can conclude Normality doesn't hold in the additive model.
\begin{figure}[htb]
    \centering
